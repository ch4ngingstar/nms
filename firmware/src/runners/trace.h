// trace recon runner (spec §6.4, §10.1; protocol result key "hops"). ICMP
// traceroute with incrementing IP TTL, reading time-exceeded from intermediate
// hops. Requires a raw ICMP socket (CONFIG_LWIP_RAW); §10.1 says that if raw
...
// Default OFF until raw-socket support is confirmed on the board (spec §10.1's
